...
class GromacsEngine(ExternalEngine):
    ...
    options : dict
        ...
        are
            ...
            * ``mdrun_args``: Additional arguments to ``mdrun``. The
              defaults take ``-s topol.tpr -o self.output_file
              -e self.edr_file -g self.log_file``, where the ``topol.tpr``
    ...
    SnapshotClass = ExternalMDSnapshot
    InternalizedSnapshotClass = InternalizedMDSnapshot
    def __init__(self, gro, mdp, top, options, base_dir="", prefix="gmx", gen_vel_mdp=None):
        ...
        if gen_vel_mdp is not None:
            # TODO: document this
            self.gen_vel_mdp = os.path.join(base_dir, gen_vel_mdp)
            self.gen_vel_mdp_contents, self._gen_vel_mdp_hash = ensure_file(self.gen_vel_mdp)
        else:
            self.gen_vel_mdp = None
            self.gen_vel_mdp_contents = None
        ...
        dirs = [self.prefix + s for s in ['_trr', '_log', '_edr', '_conf']]
        for d in dirs:
            try:
                os.mkdir(d)
            except OSError:
    ...
    def to_dict(self):
        ...
        local_dct = {
            ...
            'gen_vel_mdp_contents': self.gen_vel_mdp_contents,
        }
    ...
    def from_dict(cls, dct):
        ...
        if dct['gen_vel_mdp'] is not None:
            contents = dct.pop('gen_vel_mdp_contents')
            _ = ensure_file(filename=dct['gen_vel_mdp'], old_contents=contents)
    ...
    def cleanup(self):  # pragma: no cover
        ...
        # NOTE: keeping the tpr and the mdout could be a good idea
        # they contain all the implicit defaults that come in between the mdp
        # and the simulation (and these vary depending on the gromacs version)
        _remove_file_if_exists(self.tpr_file)
        _remove_file_if_exists(self.mdout_file)

    def engine_command(self):
        # TODO: prev_traj and next_traj are not part of mdrun_args?
    ...
    def randomize_velocities(self, snapshot, beta=None, subset_mask=None):
        ...
        # for now we will use a seperate mpd file (gen_vel_mdp)
        # the user is responsible for ensuring that the contents
        # of the two mdps (mdp and gen_vel_mdp) only differs in gen-vel paramters
        # and continuation (if we want to apply constraints here!)
        # TODO: document these caveats
        if self.gen_vel_mdp is None:
            raise ValueError("Need seperate gen_vel_mdp file.")
        file_prefix = self.filename_setter()
        self.set_filenames(file_prefix)
        self.write_frame_to_file(self.input_file, snapshot, "w")
        # ensure that the files haven't changed before we run trajectory
        _ = ensure_file(self.gro, self.gro_contents, self._gro_hash)
        _ = ensure_file(self.gen_vel_mdp, self.gen_vel_mdp_contents,
                        self._gen_vel_mdp_hash)
        _ = ensure_file(self.top, self.top_contents, self._top_hash)

        # modify grompp command: replace mdp with gen_vel_mdp
        CMD = self.GROMPP_CMD.replace('-f {e.mdp}', '-f {e.gen_vel_mdp}')
        # and run grompp
        cmd = CMD.format(e=self)
        logger.info(cmd)
        run_cmd = shlex.split(cmd)
        return_code = psutil.Popen(run_cmd, preexec_fn=os.setsid).wait()

        # now generate a one frame trajectory, i.e. starting configuration only
        # modify mdrun command by adding -nsteps 0
        CMD = self.MDRUN_CMD + ' -nsteps 0'
        cmd = CMD.format(e=self, mdrun_args=self.options['mdrun_args'])
        logger.info(cmd)
        run_cmd = shlex.split(cmd)
        # NOTE that we wait here for gromacs to finish
        # to use generate_next_fame() we would need to copy part of
        # the .start()-logic
        return_code = psutil.Popen(run_cmd, preexec_fn=os.setsid).wait()
        snap = self.read_frame_from_file(self.output_file, 0)
        if not isinstance(snap, BaseSnapshot):
            # I think it should be save to assume it is a snapshot
            # since we waited for gromacs to run through,
            # i.e. we do not need to check?
            raise OSError("Something went wrong reading the velocity"
                          + " randomized snapshot.")
        _remove_file_if_exists(self.input_file)
        _remove_file_if_exists(self.tpr_file)
        _remove_file_if_exists(self.mdout_file)
